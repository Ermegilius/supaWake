...
function statusBadge(status: number | null): { text: string; color: string } {
  ...
  if (status === 401) return { text: '401 — missing API key', color: '#e67e22' };
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* Row 2: API key */}
        <input
          placeholder="API key — anon or publishable key from Supabase → Project Settings → API  (optional but required to get 200)"
          value={apiKey}
          onChange={e => setApiKey(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && add()}
          style={{ ...inputStyle, flex: 'unset', width: '100%', boxSizing: 'border-box', marginBottom: 8, fontFamily: 'monospace', fontSize: 12 }}
        />
                ...
                <li key={p.id} style={rowStyle}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
                      {p.label || p.ref}
                      {!p.api_key && (
                        <span style={{ fontSize: 11, color: '#e67e22', fontWeight: 400 }}>no API key</span>
                      )}
                    </div>
                    {p.label && (
